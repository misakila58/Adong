...
public class DialougeManager : MonoBehaviour
{
    ...
    public void DialogueText() // 다이얼로그 시작 부분에서 해당 함수를 실행 시켜주면 됨 
    {
        ...
        if (endTextCheck == true)
        {
            ...
            {
                case 1:
                    dialougeImage.sprite = clearImage; //왼쪽 이미지 창 
                    originDialogue = "아펜도스 마을은 평화로운 빛의 가호가 내려진 많은 마을들 중 하나다." + "\n" +
                        "알렌은 그런 마을의 평범한 가정에서 자란 사냥꾼이었다. 그러던 어느 날 마을에 어두운 그림자가 내려 앉게 된다."; // 나올 텍스트 
                    StartCoroutine(TypingAction()); // 시작 
                    endTextCheck = false;
                    break;
                case 2:
                    dialougeImage.sprite = clearImage;
                    originDialogue = "알렌" + "\n" + "\"우리 마을이 왜 불타고 있는거지..?\"";
                ...
                case 3:
                    ...
                    originDialogue = "난장판이 된 집 앞에 도착한 알렌은 불길한 예감에 휩싸여 늘 가지고 다니던 석궁을 단단히" +
                        "쥐고 주변을 살폈다. 불쌍한 알렌! 그는 이미 잔인한 운명으로 한걸음 다가서고 있었다.";
                ...
                case 4:
                    dialougeImage.sprite = clearImage;
                    originDialogue = "그가 마주한 광경은 참혹했다. 괴물들이 이곳 저곳을 뒤지며 으르렁거렸고, 고깃덩이를 씹어 삼키느라 정신이 없었다." +
                ...
                case 5:
                    dialougeImage.sprite = EnemyImage;
                    originDialogue = "나약한 눈물은 흐르지 않았다. 오직 불타오르는 증오가 그의 가슴에 자리잡아 방아쇠를 당겼다." +
                ...
                case 8: 
                    ...
                    originDialogue = "Dr." +
                        "\n" +
                        "공포를 찾는 방랑자가 또 겁없이 기어들어왔구나. 그래, 내가 친히 풀어준 죽음이 마음에 드는가?"; // 나올 텍스트 
                    StartCoroutine(TypingAction()); // 시작 
                    endTextCheck = false;
                    break;
                case 9:
                ...
                case 10:
                    ...
                    originDialogue = "Dr." +
                        ...
                        "아름답지 않은가! 나를 원망하라, 복수심에 찬 이방인이여! 차오른 네 절망과 고통은 지루함에 한숨짓던 나날들의 새로운 기쁨이 될 지니."; // 나올 텍스트 
                    StartCoroutine(TypingAction()); // 시작 
                    endTextCheck = false;
                    break;
                case 11:
                ...
                case 13: // 보스전 승리 시 다이얼로그 시작 TextNum 을 12로 설정 해줘야 함 
                    ...
                    originDialogue = "Dr." +
                        "\n" +
                        "하하...통쾌한 복수로다! 네 화살이 나를 죽였따. 그러나 가소롭구나, 변두리의 영웅이여...작은 승리에 취해 세상의 혼돈을 외면하겠나?";
                    StartCoroutine(TypingAction()); // 시작 
                    endTextCheck = false;
                    break;
                case 14: 
                ...
                case 18:
                    ...
                    originDialogue = "Dr." +
                        "\n" +
                        "증오는 낡아 허물어졌고, 화살은 살가죽 하나 뚫지 못할 정도로 무뎌 졌구나. 어리석고 가엾은 자여! 네 덕에 나는 또 하염없이 기쁨을 기다려야 하나니..";
                    StartCoroutine(TypingAction()); // 시작 
                    endTextCheck = false;
                    break;
